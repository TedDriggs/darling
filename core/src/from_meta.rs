...
pub trait FromMeta: Sized {
    ...
    fn from_expr(expr: &Expr) -> Result<Self> {
        match *expr {
            ...
            _ => {
                if let Some((tokens, expr_error)) = expr_verbatim::decode(expr) {
                    if let Some(handled) = Self::from_verbatim(tokens) {
                        handled
                    } else {
                        Err(Error::from_syn_rendered(expr_error.clone()))
                    }
                } else {
    ...
    ///
    /// Anything that failed to parse as a [`syn::Expr`] will be sent to this function
    /// as the `tokens`, for example `pub(in crate::module)` which isn't a valid `Expr` but
    /// it could be manually parsed into a [`syn::Visibility`]
    ///
    /// If this function returns `None`, that means this type does not support verbatim
    /// so the default expression error will be surfaced
    ///
    /// It is recommended to implement [`syn::parse::Parse`] for your type, then
    /// have an implementation of `from_verbatim` like this:
    ///
    /// ```ignore
    /// fn from_verbatim(tokens: &TokenStream) -> Option<darling::Result<Self>> {
    ///     Some(syn::parse2(tokens.clone()).map_err(Into::into))
    /// }
    /// ```
    ///
    /// Note that the default implementation of `from_expr` will call
    /// `from_verbatim` **if, and only if** the input is an *invalid Rust expression*.
    ///
    /// If you were to try and re-implement `impl FromMeta for syn::Type`,
    /// you will run into the problem that `(i32, u32)` is both a valid [`Expr`]
    /// and a valid [`Type`](syn::Type).
    ///
    /// To truly accept a completely arbitrary list of tokens, you should implement
    /// `from_expr` as follows:
    ///
    /// ```ignore
    /// fn from_expr(expr: &Expr) -> Result<Self> {
    ///     match *expr {
    ...
    ///         _ => {
    ///             let tokens = if let Some((tokens, _)) = darling::util::decode_expr_verbatim(expr) {
    ///                 tokens.clone()
    ///             } else {
    ...
    /// ```
    #[allow(unused_variables)]
    fn from_verbatim(tokens: &TokenStream) -> Option<Result<Self>> {
        None
    }
}
        ...
        impl FromMeta for $ty {
            ...
            fn from_expr(expr: &Expr) -> Result<Self> {
                match *expr {
                    ...
                    _ => {
                        if let Some((tokens, _)) = expr_verbatim::decode(expr) {
                            Ok(syn::parse2(tokens.clone())?)
                        } else {
                            ...
                            Ok(syn::parse2(expr.into_token_stream().clone())?)
                        }
                    }
                }
                .map_err(|e| e.with_span(expr))
...
mod tests {
    ...
    use quote::quote;
    use syn::{
        parse_quote, Type, TypeArray, TypeBareFn, TypeImplTrait, TypeInfer, TypeNever, TypeParen,
        TypePtr, TypeReference, TypeSlice, TypeTraitObject, TypeTuple, Visibility, WhereClause,
    };
